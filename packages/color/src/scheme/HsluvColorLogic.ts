...
export function polynomial_scale(exp: number, start: number, end: number, size: number): number[] {
	const base = 1 / (size - 1);
...
export function polynomial_hsl_scale(
	...
	for (let i = 0; i < size; i++) {
		hexvalues.push([hues[i] as number, saturations[i] as number, luminances[i] as number]);
	}
	return hexvalues;
}
...
function getBackgroundSaturationaAndLuminance(hue: number, backgroundLevel: number, light: boolean): [number, number] {
	...
	if (light) {
		if (backgroundLevel < 50) {
			...
			const l = 100 - maxLightLuminanceOffset * (1 - (backgroundLevel - 50) / 50);
...
export class ColorMaker {
	...
	public constructor(
		...
		const [s, l] = getBackgroundSaturationaAndLuminance(h, this.backgroundLevel, this.light);
		...
		const faintLuminance = this.light ? (100 - lightestGrey) / 2 + lightestGrey : darkestGrey / 2;
	...
	public nominal(size: number): NominalHueSequence {
		if (!this.cachedNominalSequence || this.cachedNominalSequence.size !== size) {
			const { nominalBold, nominal, nominalMuted, nominalHues } = this.getNominalHueSequences(size);
	...
	private explicitGrey(size: number, startLuminance: number, endLuminance: number) {
	...
	private getAccentsAndComplements(
		...
		const maxSaturationAccent: HSLVector = [accentHue, maxSaturation, colorLuminance];
		const maxSaturationComplement: HSLVector = [complementHue, maxSaturation, colorLuminance];
	...
	public sequential(size: number, offset = 0): HSLVector[] {
		const { greyAccent, maxSaturationAccent } = this.getAccentsAndComplements(size, offset);
		return polynomial_hsl_scale(linearExponent, greyAccent, maxSaturationAccent, size);
	}

	public sequentialComplement(size: number, offset = 0): HSLVector[] {
		const { greyAccent, maxSaturationComplement } = this.getAccentsAndComplements(size, offset);
		return polynomial_hsl_scale(linearExponent, greyAccent, maxSaturationComplement, size);
	}

	public diverging(size: number, offset = 0): HSLVector[] {
		const { greyAccent, greyComplement, maxSaturationAccent, maxSaturationComplement } = this.getAccentsAndComplements(
			size,
			offset,
		);
		...
		let diverging = polynomial_hsl_scale(polynomialExponent, greyComplement, maxSaturationComplement, cut)
			.slice(1)
			.reverse();

		let toAdd = polynomial_hsl_scale(polynomialExponent, greyAccent, maxSaturationAccent, cut);
		if (size % 2 === 0) {
	...
	private getNominalHueSequences(size: number) {
		...
		const initialHues = this.nominalHueStep <= 10 ? 13 - this.nominalHueStep : this.nominalHueStep - 8;
...
export function getScheme(
	...
	const { accentHue, accentSaturation, accentLuminance, backgroundLevel, backgroundHueShift, nominalHueStep } = params;
	...
	return {
		...
		lowMidContrastAnnotation: hsluv2hex(...colorMaker.lowMidContrastAnnotationHsl),
		midContrastAnnotation: hsluv2hex(...colorMaker.midContrastAnnotationHsl),
		midHighContrastAnnotation: hsluv2hex(...colorMaker.midHighContrastAnnotationHsl),
