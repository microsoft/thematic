...
describe('color conversion functions', () => {
	...
	describe('css => rgba vector', () => {
		test('default opacity in hex string', () => {
			...
			expect(value).toEqual([0.12156862745098039, 0.4666666666666667, 0.7058823529411765, 1]);
		});

		test('override opacity with alpha param', () => {
			const value = css2rgbaVector('#1f77b4', 0.5);
			expect(value).toEqual([0.12156862745098039, 0.4666666666666667, 0.7058823529411765, 0.5]);
		});
