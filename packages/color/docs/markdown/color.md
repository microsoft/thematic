...
|  [nearest(color, list)](./color.nearest.md) | <p>Finds the closest match from one color within a list.</p><p>This just uses the Hue component from the HSLuv variant. The idea is that we just want the visually closest color.</p><p>If you need something like Euclidean distance use chrome-js.distance directly.</p> |
