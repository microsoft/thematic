...
export const ThemeColors: FC<ThemeColorsProps> = ({ scaleItemCount }) => {
	...
	const [accentColor, foregroundColor, backgroundColor, errorColor] = usePalette();
	const accent = (
		<Fragment>
	...
	const background = (
		<Fragment>
			<span style={{ color: foregroundColor! }}>Background {backgroundColor}</span>
		</Fragment>
	);
	const foreground = (
		<Fragment>
			<span style={{ color: foregroundColor! }}>Foreground {foregroundColor}</span>
		</Fragment>
	);

	const [nominal, nominalBold, nominalMuted, sequential, diverging, sequential2, diverging2, greys] =
			...
			>
				...
				>
					<li>
						...
						<Contrast foreground={foregroundColor!} background={backgroundColor!} error={errorColor!} showLink={true} />
						)
					</li>
					<li>
						{accent} on {background} (contrast ratio:{' '}
						<Contrast foreground={accentColor!} background={backgroundColor!} error={errorColor!} showLink={true} />
						)
					</li>
				</ul>
				...
				>
					<ColorStrip label="Nominal " foreground={foregroundColor!} background={backgroundColor!} colors={nominal!} />
					<ColorStrip
						label="Nominal+"
					...
					/>
					<ColorBand label="Sequential" foreground={foregroundColor!} colors={sequential!} width={BAND_WIDTH} />
					<ColorBand label="Sequential2" foreground={foregroundColor!} colors={sequential2!} width={BAND_WIDTH} />
					<ColorBand label="Diverging" foreground={foregroundColor!} colors={diverging!} width={BAND_WIDTH} />
					<ColorBand label="Diverging2" foreground={foregroundColor!} colors={diverging2!} width={BAND_WIDTH} />
					<ColorBand label="Greys" foreground={foregroundColor!} colors={greys!} width={BAND_WIDTH} />
				</div>
			</div>
		</div>
	);
