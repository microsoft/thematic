...
export function usePaletteWidth(width: number): number {
	...
	return width - ITEM_BORDER_MODIFIER - ITEM_LEFT_PADDING - TEXT_WIDTH - CARET_PADDING - ITEM_BORDER_MODIFIER;
}
...
export function useScale(key: string, width: number): NominalColorScaleFunction | ContinuousColorScaleFunction {
	const theme = useThematic();
	const scale = useMemo(() => chooseScale(theme, key, width), [theme, key, width]);
	return scale;
}
