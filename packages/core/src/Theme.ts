...
import { applyParams, applyScheme, computeDefinition, createScheme } from './scheme.js';
import type {
	Application,
	Arc,
	Area,
	...
	public scales = (): ColorScales => {
		return {
			nominal: (sizeOrDomain?: number | string[] | number[]) => {
				const size = typeof sizeOrDomain === 'number' ? sizeOrDomain : sizeOrDomain ? sizeOrDomain.length : 10;
				const domain = typeof sizeOrDomain !== 'number' ? sizeOrDomain : undefined;
			...
			sequential: (domain: number[] = [0, 1], scaleType?: ScaleType, quantiles?: number) => {
				const scheme = this.getScheme(100);
				return continuous(scheme.sequential, domain, scaleType, quantiles);
			},
			sequential2: (domain: number[] = [0, 1], scaleType?: ScaleType, quantiles?: number) => {
			...
			diverging: (domain: number[] = [-1, 1], scaleType?: ScaleType, quantiles?: number) => {
				const scheme = this.getScheme(100);
				return continuous(scheme.diverging, domain, scaleType, quantiles);
			},
			diverging2: (domain: number[] = [-1, 1], scaleType?: ScaleType, quantiles?: number) => {
				const scheme = this.getScheme(100);
				return continuous(scheme.diverging2, domain, scaleType, quantiles);
			},
			greys: (domain: number[] = [0, 1], scaleType?: ScaleType, quantiles?: number) => {
				const scheme = this.getScheme(100);
