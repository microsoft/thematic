...
export const office: Transformer<OfficeTheme> = (theme: Theme) => {
	...
	return {
		dark1: theme.dark ? theme.application().background().hex() : theme.text().fill().hex(),

		light1: theme.dark ? theme.text().fill().hex() : theme.application().background().hex(),
		dark2: theme.dark ? theme.plotArea().fill().hex() : theme.application().accent().hex(),
		light2: theme.dark ? theme.application().accent().hex() : theme.plotArea().fill().hex(),
